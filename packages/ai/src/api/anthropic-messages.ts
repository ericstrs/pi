 ...
 */
type AnthropicToolResultContent = NonNullable<ToolResultBlockParam["content"]>;
type AnthropicToolResultContentBlock = Exclude<
	Exclude<AnthropicToolResultContent, string>[number],
	{ type: "tool_reference" }
>;
type ConvertedToolResultContent = string | AnthropicToolResultContentBlock[];

function convertContentBlocks(content: UserContent[]): ConvertedToolResultContent {
	const hasMedia = content.some((c) => c.type !== "text");
	if (!hasMedia) {
		return sanitizeSurrogates(content.map((c) => (c as TextContent).text).join("\n"));
	}

	const blocks: AnthropicToolResultContentBlock[] = content.map((block) => {
		if (block.type === "text") {
			return {
				type: "text",
