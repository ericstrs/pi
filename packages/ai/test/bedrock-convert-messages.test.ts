...
describe("bedrock convertMessages handles unknown content types", () => {
	it("serializes unknown user content blocks as explicit placeholders", async () => {
		...
		expect(p.messages[0].content[1]).toEqual({
			text: "(unknown omitted: Bedrock serializer does not support native unknown)",
		});
	});
	...
	it("serializes user messages with only unknown content blocks as placeholders", async () => {
		const messages: Message[] = [
			{
				role: "user",
