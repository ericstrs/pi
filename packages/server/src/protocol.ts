import {
	...
	type PdfContent as AiPdfContent,
...
type _AiPdfContentFieldsAccountedFor = Assert<ExactKeys<AiPdfContent, "type" | "data" | "mimeType" | "name">>;
type _AiVideoContentFieldsAccountedFor = Assert<ExactKeys<AiVideoContent, "type" | "data" | "mimeType" | "name">>;
type _AiAudioContentFieldsAccountedFor = Assert<ExactKeys<AiAudioContent, "type" | "data" | "mimeType" | "name">>;
...
function toProtocolToolContent(content: ToolResultMessage["content"]): ToolTranscriptItem["content"] {
	return content.map((part) => {
		switch (part.type) {
			case "text":
				return { type: "text", text: part.text };
