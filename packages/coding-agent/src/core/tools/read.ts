...
import type { Api, Model, UserContent } from "@earendil-works/pi-ai";
...
export interface ReadOperations {
	...
	/** Detect rendered-media MIME type, return null or undefined for text/unknown files */
...
type MediaKind = Exclude<UserContent["type"], "text">;

function mediaKindFromMimeType(mimeType: string): MediaKind | null {
...
function mediaName(kind: MediaKind): string {
	return kind === "pdf" ? "PDF" : kind;
}

function unsupportedMediaName(kind: MediaKind): string {
	return kind === "image" ? "images" : mediaName(kind);
}

function getUnsupportedMediaNote(model: Model<Api> | undefined, kind: MediaKind): string | undefined {
	if (!model || model.input.includes(kind)) {
		return undefined;
	}
	const name = mediaName(kind);
	return `[Current model does not support ${unsupportedMediaName(kind)}. The ${name} will be omitted from this request.]`;
}
...
export function createReadToolDefinition(
	...
	return {
		name: "read",
		label: "read",
		description: `Read the contents of a file. Supports text files and rendered media (images, PDFs, video, audio). Rendered media is sent as native attachments. Images may be resized for model compatibility. For text files, output is truncated to ${DEFAULT_MAX_LINES} lines or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). Use offset/limit for large files. When you need the full file, continue with offset until complete.`,
			...
			return new Promise<{ content: UserContent[]; details: ReadToolDetails | undefined }>((resolve, reject) => {
				...
				(async () => {
					try {
						...
						if (aborted) return;
						const mimeType = ops.detectMediaMimeType
							? await ops.detectMediaMimeType(absolutePath)
